        ...
        if ser.in_waiting:
            try:
                ...
                if msg:
                    ...
                    if True :#ok_event.is_set():  # only set wait_event if ok was received before
                        msg_queue.put(msg)
                        if msg == "ok":
                            wait_event.set()                            
...
def _connect(port, baudrate=115200):
    ...
    logger.info(f"Starting init sequence...")
    _send_gcode("M155 S0",wait=True)
