...
def gen():
  ...
  while True:
        acquired = icam_lock.acquire(timeout=0.01)  # try for 10ms
        if not acquired:
            yield()  # skip this iteration if the lock can't be acquired quickly
        try:
...
def exposure():
    try:
        ...
        if request.method == 'POST':
            ...
            # Try form data (including legacy support)
            elif 'value' in request.form:
                value = request.form.get('value')
            elif 'text_exposure' in request.form:
                value = request.form.get('text_exposure')

            if value is None:
                return jsonify({
                    "status": "error",
                    "message": "Missing 'value' or 'text_exposure' parameter"
                }), 400
            ...
            if 0 <= value <= max_exp:
                icam.ExposureTime.SetValue(value)
                print(">>>> seting", value)
                current_exp = icam.ExposureTime.Value  # update after setting
            else:
                return jsonify({
                    "status": "error",
                    "message": f"Exposure must be between 0 and {max_exp}",
                    "max": max_exp
                }), 400

        # GET or POST: return current value and max
        return jsonify({
            "status": "ok",
